...
print("\nThis game uses the time.sleep() function for tiny delays between text, but you may turn it off.")
sleepEnabled: str = str(input("Would you like to enable the time.sleep() function? (Y/n)\n> "))
if sleepEnabled.lower() == "n":
    def sleep(awawa: float):
        pass
else:
...
def character_creation():
    ...
    if len(name) <= 0:
        ...
        names: list = ["tesla cyber", "goblin warrior", "nintendo ninja", "dell optiplex", "ford focus", "subject number five"]
...
but you can move aroundCount a bit better.")
    else:
        armour = 3
        luck += 4
        hp += 40
...
good about simplicity?")
    ...
    if armour == 1 and weapon == 1:
        ...
        # it deals very high damage, but doesnt survive easily. pure classes deal critical damage to knights.
    if armour == 2 and weapon == 3:
        ...
        # this class is quite well roundCounted. it has better survivability than the knight,
        ...
        # these crits only work on the night class. the pure class is also very tanky.
...
class characters():
    ...
    # rather than a chance to become a god and/or dodge everything. yeah idk why i made that a thing either
    name: str
    armour: str
    ...
    # yes i know weapon is an argument. no i dont care. the game runs fine and im too lazy to clean it.
    def action_attack(self, name, atk, crit, luck, name2, hp2, def2, armour2,
        ...
        if randint(2, 8) < luck2 and (randint(1, 100) + 10) < (luck2 * 10):
            print(f"{name2} got lucky! The damage they take has been reduced!")
            sleep(0.2)
            damage_dealt / 1.8

        round(damage_dealt, 1)
        damage_dealt -= def2 // 3
        hp2 -= damage_dealt
        if hp2 > 0:
    ...
    def action_defend(self, name, defence):
        if self.isDefending == 0:
            defence += 20
            self.defenceBoost = 20
            print(f"{name} is blocking the next attack, raising \
their defence by {20 - self.isDefending}! ({defence} defence)")
            self.isDefending += 1
            sleep(0.4)
        else:
            print("You already have an active block! Continuing to fortify it will give less defence.")
            defence += 20 - self.isDefending
...
their defence by {20 - self.isDefending}! ({defence} defence)")
    ...
    def action_item(self, isTurnOver):
        print("lol this isnt coded yet")
        sleep(0.2)
        print("im probably gonna implement a random item list with temporary buffs and/or healing")
        sleep(0.4)
        choice = str(input("""ANyways pick an item
    1. Super Mushroom
    ...
    Your choice (enter a corresponding number): """))
        ...
        if choice == "1":
            self.hp += 100
            print("WOW!! you healed for 100hp!")
            sleep(0.3)
            isTurnOver = True
        elif choice == "2":
            self.hp -= 30
            ...
            if rockRoll == 1:
                self.defence += 20
                print("ok what the fuck is going o-")
                sleep(0.1)
                print("DEFENCE UP 20!!")
                sleep(0.2)
            else:
                self.hp -= 25
                self.defence * 0.8
                print("ok what the fu- ow..")
                sleep(0.1)
                print("HP DOWN 25!!")
                sleep(0.2)
                print("DEFENCE DOWN 20%!!")
                sleep(0.2)
                if self.hp <= 0:
                    print("...")
                    sleep(1)
            ...
            if potionRoll == 1:
                ...
                self.hp += 120
                print("It was a health potion! hp UP 120!!")
                sleep(0.1)
                print("HP UP 120!!")
                sleep(0.2)
            elif potionRoll == 3:
                self.isDefending = 0
                self.defenceBoost = 0
                print("BONUS!! Your block state was reset, keeping your bonus defence and rendering it indestructable!")
                sleep(0.3)
            elif potionRoll == 4:
                self.luck -= 8
...
while not game_over:
    ...
    try:
        ...
        if PR.hp <= 0 or PR2.hp <= 0:
            print("An error occured! Error code: IBHP1")
        if player_choice == "1":
            PR2.hp, game_over, changeTurn, PR2.defenceBoost, PR2.isDefending, PR2.defence = PR.action_attack(PR.name, PR.atk, PR.crit, PR.luck, PR2.name, PR2.hp, PR2.defence, PR2.armour, PR2.luck, game_over, PR2.defenceBoost, PR2.isDefending)
        elif player_choice == "2":
            PR.defence, changeTurn = PR.action_defend(PR.name, PR.defence)
        elif player_choice == "3":
            changeTurn = PR.action_item(changeTurn)
        elif player_choice == "4":
            changeTurn = PR.action_viewstats(PR.hp, PR.atk, PR.defence, PR.luck, PR.playerIsClass, changeTurn)
        else:
            print("That's not a valid option! Try again!")
            changeTurn = False
            continue
    except (TypeError):
        print("Something went wrong! But the game should continue just fine. (Error code: TPF)")
    finally:
        if changeTurn:
            if turn == 1:
                turn += 1
            ...
            continue
    if PR.hp <= 0 and PR2.hp <= 0:
        print("An error occured.")
        sleep(0.2)
        print("Error code: both_players_dead\nPlease report this on Github.")
        sleep(5)
        exit()
